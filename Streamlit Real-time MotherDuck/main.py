...
def get_data():
    """
    Executes a SQL query on a database connection (`conn`) and returns the result
    as a pandas DataFrame (`df`). The query selects `page_id` and `count` from a
    table named by `table_name`, ordered by `page_id` in ascending order, and
    prints the execution time.

    Returns:
        pandasDataFrame: A data structure that contains tabular data.
...
def get_cached_data():
    """
    Retrieves cached data by calling `get_data`. It utilizes the `@st.cache_data`
    decorator, which caches the output of the decorated function for subsequent
    calls with the same arguments. This approach enhances performance and reduces
    computational overhead by reusing cached results.

    Returns:
        Any: Retrieved by calling the function `get_data`. The return value may
        be cached to speed up subsequent calls.
